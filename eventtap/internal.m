...
typedef enum {
  legacyCallbackBehavior = 0,
  observeOnly,
  consumeOnly
} callbackBehaviorType ;
...
//  is wrong in some way... but at least it's nice enough to tell us.
    ...
    bool ignoreevent = (e->fnType == consumeOnly) ; // will be changed to correct value if legacy

    if (e->fnType == legacyCallbackBehavior) {
        ...
        ignoreevent = lua_toboolean(L, -2); // see, told you we'd change it
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            LuaSkin *queueSkin = [LuaSkin shared] ;
            lua_State *queueL = queueSkin.L ;

            [queueSkin pushLuaRef:refTable ref:e->fn];
            new_eventtap_event(queueL, copy);
            CFRelease(copy);

            if (![queueSkin protectedCallAndTraceback:1 nresults:0]) {
                const char *errorMsg = lua_tostring(queueL, -1);
                if (!errorMsg) {
                    [queueSkin logBreadcrumb:[NSString stringWithFormat:@"ERROR: eventtap_callback callback returned something that isn't a string: %d", lua_type(queueL, -1)]];
                } else {
                    [queueSkin logError:[NSString stringWithFormat:@"hs.eventtap callback error: %s", errorMsg]];
                }
                lua_pop(queueL, 1) ; // remove error message
            }
        }) ;
    }
...
/// hs.eventtap.new(types, fn, [type]) -> eventtap
/// Constructor
...
///  * type - an optional boolean or explicit nil specifying the callback's return argument behavior.  Defaults to `nil`.  The behaviors are as follows:
///    * `nil`   - the callback function is executed synchronously and the return values should follow the guidelines given for the `fn` type described above.  This is the default for backwards compatibility.
///    * `false` - the callback function will observe only and should return no values.  The callback will be executed asynchronously.
///    * `true`  - the callback function will consume the event and should return no values.  The callback will be executed asynchronously.
///
/// Returns:
///  * An event tap object
///
...
///
///  * A callback function which is assigned the type of `true` or `false` is executed asynchronously to maximize performance.  Because such a callback cannot return any values, if you wish to modify or provide additional events based upon the event data evaluated by the callback, you will need to explicitly post your own events with `hs.eventtap.event:post(...)` within the callback function.
static int eventtap_new(lua_State* L) {
    LuaSkin *skin = [LuaSkin shared];
    [skin checkArgs:LS_TTABLE, LS_TFUNCTION, LS_TBOOLEAN | LS_TNIL | LS_TOPTIONAL, LS_TBREAK];
    callbackBehaviorType callbackType = legacyCallbackBehavior ;
    if ((lua_gettop(L) == 3) && (lua_type(L, 3) == LUA_TBOOLEAN)) {
        callbackType = lua_toboolean(L, 3) ? consumeOnly : observeOnly ;
    }
