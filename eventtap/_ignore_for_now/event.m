...
/// hs.eventtap.event.newFlagChangeEvent(mods, target, [isLeft]) -> event
/// Constructor
/// Creates a flag(s) change event (keyboard modifiers)
///
/// Parameters:
...
///  * target - a string matching one of the key names described above specifying which specific modifier this event is for.  Note that the modifier specified for this argument should not be present in the `mods` table if this is to be considered a "keyUp" or release of the modifier.
///  * isLeft - an optional boolean, default true, specifying whether or not the target specifies the left (true) or right (false) key.  Ignored for `fn` and `capsLock`.
///
/// Returns:
///  * An `hs.eventtap.event` object
///
/// Notes:
///  * When one or more modifiers are pressed on a keyboard, a series of events are generated, one for each modifier key pressed.  The event order is unspecified, but each event contains one more of the specified modifiers until the final event which contains all of the modifiers combined.  As the modifiers are released, a series of events are generated, each one containing one fewer modifier until the last event which contains no modifiers.
///  * This constructor creates a single event which can serve as one of the steps in this build-up or tear-down of modifiers.
///  * See [hs.eventtap.event.postFlagChangeEvents](#postFlagChangeEvents) to automate the simulation of pressing or releasing one or more modifier flags at the same time.
///
///  * It is possible that unexpected behavior may occur if you bypass the proper build-up/tear-down and just create a single event for specifying multiple modifiers being pressed or released, so consider using [hs.eventtap.event.postFlagChangeEvents](#postFlagChangeEvents) unless you know what you are doing.
///
///  * The example given in [hs.eventtap.event.newKeyEvent](#newKeyEvent) utilizing this function:
///  ~~~lua
///      hs.eventtap.event.newFlagChangeEvent( { shift = true }, "shift"):post()
///      hs.eventtap.event.newFlagChangeEvent( { shift = true, alt = true }, "alt"):post()
///      hs.eventtap.event.newKeyEvent("a", true):post()
///      hs.eventtap.event.newKeyEvent("a", false):post()
///      hs.eventtap.event.newFlagChangeEvent( { shift = true }, "alt"):post()
///      hs.eventtap.event.newFlagChangeEvent( { }, "shift"):post()
///  ~~~
static int eventtap_event_newFlagChangeEvent(lua_State* L) {
    LuaSkin *skin = [LuaSkin shared];
    [skin checkArgs:LS_TTABLE, LS_TSTRING, LS_TBOOLEAN | LS_TOPTIONAL, LS_TBREAK] ;
    NSString     *targetKey = [skin toNSObjectAtIndex:2] ;
    BOOL         targetIsLeft = (lua_gettop(L) == 3) ? (BOOL)lua_toboolean(L, 3) : YES ;
    CGEventFlags modifierFlags = (CGEventFlags)0 ;
    CGKeyCode    targetKeyCode ;

    if (lua_getfield(L, 1, "cmd") != LUA_TNIL) {
        modifierFlags |= NX_COMMANDMASK | (lua_toboolean(L, -1) ? NX_DEVICELCMDKEYMASK : NX_DEVICERCMDKEYMASK) ;
    }
    lua_pop(L, 1) ;
    if (lua_getfield(L, 1, "shift") != LUA_TNIL) {
        modifierFlags |= NX_SHIFTMASK | (lua_toboolean(L, -1) ? NX_DEVICELSHIFTKEYMASK : NX_DEVICERSHIFTKEYMASK) ;
    }
    lua_pop(L, 1) ;
    if (lua_getfield(L, 1, "alt") != LUA_TNIL) {
        modifierFlags |= NX_ALTERNATEMASK | (lua_toboolean(L, -1) ? NX_DEVICELALTKEYMASK : NX_DEVICERALTKEYMASK) ;
    }
    lua_pop(L, 1) ;
    if (lua_getfield(L, 1, "ctrl") != LUA_TNIL) {
        modifierFlags |= NX_CONTROLMASK | (lua_toboolean(L, -1) ? NX_DEVICELCTLKEYMASK : NX_DEVICERCTLKEYMASK) ;
    }
    lua_pop(L, 1) ;
    if (lua_getfield(L, 1, "fn") != LUA_TNIL) {
        modifierFlags |= NX_SECONDARYFNMASK  ;
    }
    lua_pop(L, 1) ;
    if (lua_getfield(L, 1, "capsLock") != LUA_TNIL) {
        modifierFlags |= NX_ALPHASHIFTMASK  ;
    }
    lua_pop(L, 1) ;

    if ([targetKey isEqualToString:@"cmd"]) {
        targetKeyCode = targetIsLeft ? kVK_Command : kVK_RightCommand ;
    } else if ([targetKey isEqualToString:@"shift"]) {
        targetKeyCode = targetIsLeft ? kVK_Shift : kVK_RightShift ;
    } else if ([targetKey isEqualToString:@"alt"]) {
        targetKeyCode = targetIsLeft ? kVK_Option : kVK_RightOption ;
    } else if ([targetKey isEqualToString:@"ctrl"]) {
        targetKeyCode = targetIsLeft ? kVK_Control : kVK_RightControl ;
    } else if ([targetKey isEqualToString:@"fn"]) {
        targetKeyCode = kVK_Function ;
    } else if ([targetKey isEqualToString:@"capsLock"]) {
        targetKeyCode = kVK_CapsLock ;
    } else {
        return luaL_error(L, "invalid modifier key specified, must be one of cmd, shift, alt, ctrl, fn, or capsLock") ;
    }

    if (!eventSource) {
        eventSource = CGEventSourceCreate(kCGEventSourceStatePrivate);
    }
    [skin logDebug:[NSString stringWithFormat:@"%s.newFlagChangeEvent key = %d, flags == 0x%08llx", EVENT_USERDATA_TAG, targetKeyCode, modifierFlags]] ;
    CGEventRef event = CGEventCreate(eventSource);
    CGEventSetType(event, kCGEventFlagsChanged) ;
    CGEventSetFlags(event, modifierFlags);
    CGEventSetIntegerValueField(event, kCGKeyboardEventKeycode, targetKeyCode);
    new_eventtap_event(L, event);
    CFRelease(event);
...
static luaL_Reg eventtapeventlib[] = {
    ...
    {"newFlagChangeEvent", eventtap_event_newFlagChangeEvent},
