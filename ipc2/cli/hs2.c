...
static void setupcolors(void) {
    ...
    if (error) { COLOR_ERROR = CFStringCopyUTF8String(error) ; }
}

static void target_send(CFMessagePortRef port, CFMutableStringRef inputstr, bool israw) {
    CFDataRef inputData = CFStringCreateExternalRepresentation(NULL, inputstr, kCFStringEncodingUTF8, 0);

    CFDataRef returnedData;
    SInt32 code = CFMessagePortSendRequest(port, (israw ? -1 : 0), inputData, 2, recvTimeout, kCFRunLoopDefaultMode, &returnedData);
    CFRelease(inputData);
...
void usage(char *argv0) {
    printf("usage: %s [-m portName] [-i | -s | -c code] [-r] [-n]\n", argv0);
    exit(0);
}
...
int main(int argc, char * argv[]) {
    ...
    char* CFPortName = "hsCommandLine" ;

    int ch;
    while ((ch = getopt(argc, argv, "nirc:t:shm:")) != -1) {
        switch (ch) {
            ...
            case 'm': CFPortName = optarg; break;
            case 'h': case '?': default:
                usage(argv[0]);
        }
    }
    ...
    CFMessagePortRef port = CFMessagePortCreateRemote(NULL, CFStringCreateWithCString(NULL, CFPortName, kCFStringEncodingUTF8));

    if (!port) {
        fprintf(stderr, "error: can't access Hammerspoon; is it running with the ipc2 module loaded?\n");
        return 1;
    }
    ...
    if (readstdin) {
        ...
        while (1) {
            ...
            if (!CFMessagePortIsValid(port)) {
                ...
                port = CFMessagePortCreateRemote(NULL, CFStringCreateWithCString(NULL, CFPortName, kCFStringEncodingUTF8));
                if (!port) {
